...
public class C20250610 {
    ...
    public static void main(String[] args) {
        ...
        int[] anser = solution(enroll, referral, seller, amount);
        return;
    ...
    public static int[] solution(String[] enroll, String[] referral, String[] seller, int[] amount) {
        ...
        for (int i = 0; i < seller.length; i++) {
            ...
            String sell_person = seller[i];
            int total = 100 * amount[i];
            while (true) {

                if (sell_person.equals("-")) {
                    break;
                }

                int commission = Math.round(total / 10);
                answer[enrollMap.get(sell_person)] += total - commission;
                if (commission <= 0) {
                    break;
                }
                total = commission;
                sell_person = parentMap.get(sell_person);
